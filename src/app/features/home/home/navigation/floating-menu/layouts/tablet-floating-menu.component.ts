...
@Component({
  ...
  template: `
    <div class="tablet-floating-menu" [class.rtl]="isRTL" *ngIf="!isTabletLarge">
  ...
  `,
  styles: [`
    /* Global Divider Standardization for Tablet Floating Menu */
    ::ng-deep .tablet-floating-menu mat-divider {
    ...
    ::ng-deep .tablet-floating-menu.rtl mat-divider {
      background: linear-gradient(
        to left,
        transparent 0%,
        rgba(0, 27, 63, 0.08) 20%,
...
export class TabletFloatingMenuComponent implements OnInit, OnDestroy {
  ...
  // Menu mapping for tablet layout
  private menuMapping = {
    'left-btn-1': 'about-us',
    'left-btn-2': 'library',
  ...
  get isTabletLarge(): boolean {
    if (!this.childData) return false;
    return this.childData.breakpoint === 'tablet-large';
  }
  ...
  ngOnInit() {
    ...
    // Add click listener to document for clickaway functionality
    this.clickListener = (event: MouseEvent) => {
      const target = event.target as HTMLElement;
      if (!target.closest('.tablet-floating-menu')) {
  ...
  getMainMenuItems(buttonId: string): any[] {
    ...
    // For stepper dropdowns, return main menu items
  ...
  getStepperMenuItems(buttonId: string): any[] {
    ...
    // For stepper dropdowns, return stepper menu items
    if (buttonId === 'left-btn-2') {
  ...
  getStepperTitle(buttonId: string): string {
    const currentLang = this.languageService.getCurrentLanguage();
    const langCode = currentLang?.code || 'en';
    
    // Return stepper title based on button ID
    if (buttonId === 'left-btn-2') {
      const dmcCsrItem = this.floatingMenuContent.getContent().leftMenuItems.find(item => item.id === 'dmc-csr');
      return dmcCsrItem ? dmcCsrItem.label[langCode as 'en' | 'ar'] : 'DMC-CSR';
    } else if (buttonId === 'right-btn-2') {
      const contactItem = this.floatingMenuContent.getContent().rightMenuItems.find(item => item.id === 'contact');
      return contactItem ? contactItem.label[langCode as 'en' | 'ar'] : 'Contact';
    }
    return '';
  }
