...
@Component({
  selector: 'dmc-desktop-header',
  standalone: true,
  imports: [CommonModule, AsyncPipe, NeomorphicButtonComponent, LanguageSelectorComponent, MatIconModule] as const,
     template: `
     <div class="desktop-header" [ngClass]="getBreakpointClass()" role="navigation" aria-label="Desktop header quick actions" (click)="onHeaderStripClick($event)">
  ...
  styles: [`
    :host {
      display: block;
       ...
       /* Default height for all desktop sizes */
       height: 60px;
     }

           /* Container for the 5 button elements with max-width */
      .header-container {
        display: grid;
...
export class DesktopHeaderComponent implements OnDestroy {
  ...
  get whatsappButtonConfig() {
    ...
    return {
      ...
      height: '35px', // Consistent height for all desktop sizes
      fontSize: '14px', // Consistent font size for all desktop sizes
      minWidth: '120px',
      borderRadius: '25px',
      primaryColor: '#001B3F',
  ...
  constructor(
    ...
    // Setup click-away listener only in browser
  ...
  private startAutoCloseTimer(menuType: 'dropdown1' | 'dropdown2' | 'dropdown3') {
    // Only run timers in browser environment
    if (!isPlatformBrowser(this.platformId)) {
      return;
    }
  ...
  private clearAutoCloseTimer(menuType: 'dropdown1' | 'dropdown2' | 'dropdown3') {
    // Only clear timers in browser environment
    if (!isPlatformBrowser(this.platformId)) {
      return;
    }
  ...
  ngOnDestroy() {
    ...
    // Clean up click listener only in browser
  ...
  getBreakpointClass(): string {
    if (!this.childData) return 'desktop-small';
    const { breakpoint } = this.childData;
    return breakpoint === 'desktop-small' ? 'desktop-small' : 'desktop-large';
  }
